...
{% macro synapsespark__file_format_clause() %}
...
{% macro synapsespark__location_clause() %}
...
{% macro synapsespark__options_clause() -%}
...
{% macro synapsespark__comment_clause() %}
...
{% macro synapsespark__partition_cols(label, required=false) %}
...
{% macro synapsespark__clustered_cols(label, required=false) %}
...
{% macro synapsespark__create_temporary_view(relation, compiled_code) -%}
    create or replace temporary view {{ relation }} as
      {{ compiled_code }}
{%- endmacro -%}


{%- macro synapsespark__create_table_as(temporary, relation, compiled_code, language='sql') -%}
  {%- if language == 'sql' -%}
    {%- if temporary -%}
...
{% macro synapsespark__create_view_as(relation, sql) -%}
...
{% macro synapsespark__create_schema(relation) -%}
  {%- call statement('create_schema') -%}
    create schema if not exists {{relation}}
  {% endcall %}
{% endmacro %}

{% macro synapsespark__drop_schema(relation) -%}
...
{% macro synapsespark__get_columns_in_relation_raw(relation) -%}
...
{% macro synapsespark__get_columns_in_relation(relation) -%}
  {{ return(adapter.get_columns_in_relation(relation)) }}
{% endmacro %}

{% macro synapsespark__list_relations_without_caching(relation) %}
...
{% macro synapsespark__list_schemas(database) -%}
...
{% macro synapsespark__rename_relation(from_relation, to_relation) -%}
...
{% macro synapsespark__drop_relation(relation) -%}
...
{% macro synapsespark__generate_database_name(custom_database_name=none, node=none) -%}
  {% do return(None) %}
{%- endmacro %}

{% macro synapsespark__persist_docs(relation, model, for_relation, for_columns) -%}
  {% if for_columns and config.persist_column_docs() and model.columns %}
    {% do alter_column_comment(relation, model.columns) %}
  {% endif %}
{% endmacro %}

{% macro synapsespark__alter_column_comment(relation, column_dict) %}
...
{% macro synapsespark__make_temp_relation(base_relation, suffix) %}
...
{% macro synapsesparky__alter_column_type(relation, column_name, new_column_type) -%}
...
{% macro synapsespark__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
